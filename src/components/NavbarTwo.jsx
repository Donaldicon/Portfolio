...
useEffect(() => {
  if ('scrollRestoration' in window.history) {
    ...
    const navLinksClass = ({isActive}) =>   `transition-all duration-500 ease-in-out hover:text-[#2A2A2B] dark:hover:text-[#F0F0F0] ${
    isActive
      ? 'text-[#2A2A2B] dark:text-[#F0F0F0]'
      : 'text-[#848485] dark:text-[#545455]'
  }`
    ...
    <section>
      <div className='hidden bg-[#f2f2f2] dark:bg-[#171717] dark:text-white md:flex items-center justify-between py-3 px-[4%] laptop:py-6 big:px-[15%] big:py-10 shadow-lg dark:shadow-[#242323] fixed w-full z-50 transition-all duration-500 ease-in-out '>
        ...
        <div className='relative left-[50%] right-[50%] -translate-x-1/2' >
          ...
          <NavLink to='/' onClick={scrollToTop}   className={(props) =>
          `${navLinksClass(props)} ${
            enabled
              ? 'cursor-pointer'
            ...
            <NavLink to='/about' onClick={scrollToTop} className={(props) =>
          `${navLinksClass(props)} ${
            enabled
              ? 'cursor-pointer'
              : 'cursor-not-allowed pointer-events-none'
            ...
            <NavLink to='/projects' onClick={scrollToTop} className={(props) =>
          `${navLinksClass(props)} ${
            enabled
              ? 'cursor-pointer'
              : 'cursor-not-allowed pointer-events-none'
            ...
            <NavLink to={whatsappLink} onClick={scrollToTop} target='_blank' className={(props) =>
          `${navLinksClass(props)} ${
            enabled
              ? 'cursor-pointer'
              : 'cursor-not-allowed pointer-events-none'
      ...
      >
        ...
        <div>
          <div className={`flex items-center flex-col justify-center space-y-7 px-3 py-5 rounded-lg  font-jetBrainsMono text-[#848485] dark:text-[#545455] text-[11px] laptop:text-[18px] transition-opacity duration-500 ${isOpen ? "opacity-100":"opacity-0"}`}>
              <NavLink to='/' onClick={scrollToTop} className={(props) =>
          `font-semibold ${navLinksClass(props)} ${
            enabled
              ? 'cursor-pointer'
              : 'cursor-not-allowed pointer-events-none'
          }`
